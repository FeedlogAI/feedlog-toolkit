...
export class FeedlogIssuesList {
  ...
  render() {
    return h(
      Host,
      {
        key: 'd153440587f93dc99a2677b3f7ba4c76e037ee6c',
        class: this.theme === 'dark' ? 'dark' : '',
      },
      h(
        'div',
        { key: '527b319e821ab1479527d498a809e9ea538a269f', class: 'issues-list' },
        this.issues.length === 0
          ? h('div', { class: 'empty-state' }, h('p', null, 'No issues found'))
          : this.issues.map(issue =>
              h(
                'div',
                { key: issue.id, class: 'issue-card' },
                h(
                  'div',
                  { class: 'issue-content' },
                  h(
                    ...
                    h(
                      ...
                      issue.type === 'bug'
                        ? h('feedlog-badge', { variant: 'destructive' }, 'Bug')
                        : h('feedlog-badge', { variant: 'enhancement' }, 'Enhancement')
                    ),
                    issue.pinnedAt &&
                      h('div', { class: 'pinned-indicator', title: 'Pinned issue' }, '\uD83D\uDCCC')
                  ),
                  h(
                    'div',
                    { class: 'issue-main' },
                    h(
                      'div',
                      { class: 'issue-details' },
                      h('h3', { class: 'issue-title' }, issue.title),
                      h('p', { class: 'issue-body' }, issue.body),
                      h(
                        'div',
                        ...
                        issue.githubIssueNumber > 0 &&
                          h('span', { class: 'github-number' }, '#', issue.githubIssueNumber)
                      )
                    ),
                    h(
                      'button',
                      ...
                      issue.hasUpvoted ? h(HeartFilledIcon, null) : h(HeartOutlineIcon, null),
                      h('span', { class: 'upvote-count' }, issue.upvoteCount)
                    )
                  ),
                  h(
                    'div',
                    ...
                    h(
                      'span',
                      { class: 'issue-date', title: `Updated: ${issue.updatedAt}` },
                      'Updated ',
                      this.formatDate(issue.updatedAt)
                    ),
                    h(
                      'span',
                      { class: 'issue-date', title: `Created: ${issue.createdAt}` },
                      'Created ',
